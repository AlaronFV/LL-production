...
uint32_t WordIndex::get_or_create_id(const std::string& w) {
    auto it = _w2i.find(w);
    if (it != _w2i.end()) {
        return it->second;
    }
    uint32_t new_id = _i2w.size();
    _i2w.push_back(w);
    _w2i[w] = new_id;
    return new_id;
}

bool WordIndex::has_word(const std::string& w) const {
    return _w2i.count(w) > 0;
}

std::string WordIndex::get_word(uint32_t idx) const {
    if (idx >= _i2w.size()) {
        throw std::out_of_range("Word ID out of bounds");
    }
    return _i2w[idx];
}

size_t WordIndex::size() const {
    return _i2w.size();
}
...
void WordIndex::add_word_for_load(const std::string& w, uint32_t idx) {
    if (_i2w.size() <= idx) {
        _i2w.resize(idx + 1);
    }
    _i2w[idx] = w;
...
void VocabularyModel::_resize_arrays(size_t new_n) {
    ...
    // Populate priority queue for new words
    for (uint32_t i = old_size; i < size; ++i) {
        _word_decay_pq.push({0.0, i});
    }
}

void VocabularyModel::_ensure_words_exist(const std::vector<uint32_t>& word_ids) {
    size_t max_id = 0;
    if (word_ids.empty()) return;
    max_id = *std::max_element(word_ids.begin(), word_ids.end());
    
    // Ensure arrays are large enough to hold the highest ID
    if (max_id >= prof.size()) {
        _resize_arrays(max_id + 1);
    }
}
...
std::vector<float> VocabularyModel::get_effective_proficiency(const std::vector<uint32_t>& word_ids) {
    std::vector<float> out;
    out.reserve(word_ids.size());
    for (uint32_t wid : word_ids) {
        if (wid < eff_prof.size()) {
            out.push_back(eff_prof[wid]);
        } else {
            out.push_back(0.0365f); // Default for unknown
        }
    }
    return out;
}
...
float VocabularyModel::_decay_trace_activation(int trace_idx, double now_h) {
    if (trace_idx < 0 || trace_idx >= _trace_activations.size()) return 0.0f;

    double elapsed = now_h - _trace_timestamps_h[trace_idx];
    if (elapsed <= min_elapsed_h) {
...
float VocabularyModel::_get_word_activation(uint32_t word_id, double now_h) {
    if (word_to_traces.find(word_id) == word_to_traces.end()) {
        return 0.0f;
    }
...
void VocabularyModel::_process_due_trace_decays(double now_h) {
    while (!_trace_decay_pq.empty() && _trace_decay_pq.top().next_decay_time <= now_h) {
        ...
        if (trace_idx < _trace_activations.size() && _trace_activations[trace_idx] >= trace_delete_threshold) {
...
void VocabularyModel::_propagate(int source_trace_idx) {
    if (source_trace_idx < 0 || source_trace_idx >= _trace_activations.size()) return;
    ...
    while(!queue.empty()){
        ...
        for (uint32_t word_id : current_word_ids) {
            if (word_to_traces.count(word_id)) {
                for (int neighbor_trace_idx : word_to_traces.at(word_id)) {
                    if (neighbor_trace_idx != current_trace_idx) {
                        visited_neighbors[neighbor_trace_idx] = true;
                    }
                }
            }
        }
        ...
        for(auto const& [neighbor_trace_idx, _] : visited_neighbors) {
            ...
            if (tj_part < step) {
                ...
                if (step > propagation_threshold) {
                    queue.push_back({neighbor_trace_idx, step});
                }
            }
        }
    }
...
void VocabularyModel::_prune_traces() {
    ...
    for (int i = 0; i < _trace_activations.size(); ++i) {
        if (_trace_activations[i] >= trace_delete_threshold) {
            traces_to_keep_indices.push_back(i);
        }
    }
    ...
    for (int new_idx = 0; new_idx < traces_to_keep_indices.size(); ++new_idx) {
        int old_idx = traces_to_keep_indices[new_idx];
    ...
    for (int new_idx = 0; new_idx < _trace_word_ids.size(); ++new_idx) {
        for (uint32_t wid : _trace_word_ids[new_idx]) {
            word_to_traces[wid].push_back(new_idx);
        }
    }
}
...
float VocabularyModel::_calculate_context_support(const std::set<uint32_t>& id_words, size_t n) {
    ...
    float mx = 0.0f;
    for (int trace_idx = 0; trace_idx < _trace_timestamps_h.size(); ++trace_idx) {
...
float VocabularyModel::predict_understanding(const std::vector<uint32_t>& word_ids, double current_time_h) {
    ...
    std::set<uint32_t> non_zero; // CHANGED: Initialize as empty
    
    for (const auto& w : words) {
        float ep;
        if (idx.has_word(w)) {
            uint32_t wid = idx.get_id(w);
            ep = eff_prof[wid];
            non_zero.insert(wid); // CHANGED: Only add ID if word exists
        } else {
            ep = 0.0365f; // Default for unknown
        }
...
// High-level wrapper that handles string-to-ID conversion and array resizing
void VocabularyModel::update_from_words(const std::vector<std::string>& words, float u_val) {
    ...
    for(const auto& w : words) {
        // Use get_or_create_id to add new words to the vocabulary
        word_ids.push_back(idx.get_or_create_id(w));
    }
    
    // Ensure arrays are sized correctly *after* all new words have been added
    _ensure_words_exist(word_ids);

    // Call the core proficiency update logic
...
void VocabularyModel::update_proficiency(const std::vector<uint32_t>& word_ids, float u_val, double now_h) {
    ...
    float expected = predict_understanding(word_ids, now_h);
    std::set<uint32_t> wids(word_ids.begin(), word_ids.end());
    
    int new_trace_idx = _add_trace(wids, now_h, 1.0f, (0.1f - 0.05f * u_val));
    _propagate(new_trace_idx);
    _prune_traces();

    float err = u_val - expected;
    
    for (uint32_t wid : wids) {
        encounters[wid]++;
        _word_last_decay_h[wid] = now_h;
        float cp = prof[wid];
        float cv = vol[wid];
...
template<typename T>
void read_vec(std::ifstream& f, std::vector<T>& vec, size_t n) {
    vec.resize(n);
...
std::shared_ptr<VocabularyModel> VocabularyModel::load_fast(const std::string& path) {
    ...
    if (!f.is_open()) return std::make_shared<VocabularyModel>(); // Return new model if file not found
    ...
    if (version != MODEL_VERSION) {
        throw std::runtime_error("Incompatible model version. Found " + std::to_string(version) + ", expected " + std::to_string(MODEL_VERSION));
    }

    uint32_t n_words, n_traces;
